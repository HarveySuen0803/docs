...
这里追求 Pause Time, 减少了每次 GC 的时长, 用户体验好了, 但是 Throughput 下降了, Throughput = 1000 / 1400 = 71%
...
- Initial Mark, 标记 GC Roots 和 GC Roots 直接管理的第一层 Reachable Object, 仅仅是标记工作, STW 非常短
...
CMS GC 无法处理 Float Garbage. 在 Concurrent Mark 过程中, CMS GC 无法发现并发运行的 User Thread 产生的新垃圾, 这戏 Float Garbage, 需要等下次 GC 再去清理这些垃圾（刚开始通过 GC Roots 标记上了，不认为是垃圾，并行标记的过程中，这些对象不再被引用，无法在本次 GC 中进行回收）
