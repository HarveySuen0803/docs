...
通过 javap -c -v 反编译后的字节码：

```
public static void main(java.lang.String[]);
    Code:
        // stack=2 表示 Operand Stack 的最大深度为 2
        // locals=4 表示 Local Variable Table 的容量为 4
        // args_size=1 表示参数占一个 Slot
        stack=2, locals=4, args_size=1            
            ...
            13: istore_3
            14: return
        LineNumberTable:                          // java code 行号 和 byte code 行号 的对应关系
            line 5: 0                             // java code 第 5 行 对应 byte code 第 0 行
            line 6: 8
            line 7: 11
            line 8: 14
        LocalVariableTable:
            ...
            Start  Length  Slot  Name   Signature
                ...
                8       7     1   obj   Ljava/lang/Object;   // byte code 的第 7 行创建 obj，所以 Start = 7 + 1
...
由于 Dynamic Binding 的存在, 很多 Reference 需要在 Runtime 时确定, Dynamic Linking 就负责在运行时，将 Symbolic Reference 转成 Direct Reference

Symbolic Reference 是一个标识, 用于描述所引用的目标的各种符号信息, 包括类和接口的全限定名, 字段的名称和描述符, 方法的名称和描述符等, 是编译期间生成的，存储在 class file 的 Class Constant Pool
...
Static Linking 是在 Class Loading 的 Resolve 期间，将能确定的 Reference 从 Symbolic Reference 转成 Direct Reference
...
// Dog 的 vtable
// 0: speak() -> Dog.speak，覆盖了 Animal.speak
...
Exception Table 用来描述当某段代码发生异常时，应该跳转到哪里去处理。每个方法在编译成 .class 文件后，会包含一个 Code 属性，其中就有一张 Exception Table。
...
```
Exception table:
   // from 0 to 4：这段字节码是 try 块，如果发生 ArithmeticException，跳转到 7（handler_pc）执行 catch 代码
   from    to  target type
...
TLAB（Thread-Local Allocation Buffer）是一种用于提高多线程并发分配对象的机制, TLAB 对 Eden 进行划分, 每个线程私有一小份内存
...
TLAB 的大小不是固定的，而是 JVM 在运行时根据线程行为不断调整，核心目标是在 “减少分配竞争” 和 “减少空间浪费” 之间找到平衡。JVM 会统计每个线程分配对象的速率和 TLAB 浪费率，来动态调节每个线程 TLAB 的大小

每次 GC 或分配统计后，新的 TLAB 大小 ≈ Eden / 活跃线程数 × 调整因子

配置 TLAB

- `-XX:UseTLAB` 启用 TLAB，默认开启
- `-XX:+ResizeTLAB` 动态调节 TLAB size，默认开启
...
通过 intern() 访问, 则会在 SCP 中存储一个引用指向 Heap 中的 "ab", 所以 SCP 不仅存储 String Object, 还会存储 String Object Ref
...
- 采用 Bump Pointer 或 Free List 给 Object 分配内存
