...
- JDK8 的 HashMap 采用尾插法, 扩容后链表顺序不会颠倒, 避免了死循环.
...
- ﻿ConcurrentHashMap 是由多个 ﻿Segment 组成的（默认并发 16 就是 16 个 Segment）
- Segment 直接继承 ReentrantLock，内部还维护了一个 `HashEntry<K,V>[] table`，所以每个 Segment 内部维护一部分数据
- 当一个线程需要访问某个 Segment 中的元素时, 需要先获得这个 Segment 对应的 ReentrantLock, 实现了对数据的分段加锁, 这种策略是 ﻿ConcurrentHashMap 能提供比 ﻿HashTable 更好的并发性能的关键.
- 每个 key 通过 `(hash >>> segmentShift) & segmentMask` 计算得出属于哪一个 Segment

```
ConcurrentHashMap
 ├── Segment[ ]   // 分段数组
 │     ├── HashEntry[ ]  // 每个 Segment 内部是一个哈希表
 ├── Segment[ ]   // 分段数组 
```
...
CounterCell[] 元素个数会随着并发量逐步提高，最高达到 Cpu Core 的个数，每个线程通过 `(counterCells.length - 1) & probe` 计算得出自己需要操作哪一个 CounterCell，每个线程完成 HashMap 的修改操作后，会回来修改 CounterCell 的 value 值，保证最后统计的 size() 无误。

ConcurrentHashMap 扩容流程：

1. 触发扩容条件，当元素数量 ≥ sizeCtl（≈ capacity × 0.75）时，某个线程在 put 时检测到需要扩容。

2. CAS 修改 sizeCtl，发起扩容，线程通过 CAS 将 sizeCtl 设为负值，表示“正在扩容”，并记录参与扩容的线程数。

3. 创建新表 nextTable，分配一个容量为原来 2 倍 的新数组，用于存放迁移后的数据。

4. 初始化迁移任务分配器，设置全局变量 transferIndex，用于让多个线程分段领取要迁移的桶区间。

5. 多线程协作迁移数据

- 当前线程 + 其他参与线程（包括后续 put 线程）
- 通过 transferIndex 领取一段桶区间
- 并行执行迁移（transfer）

6. 迁移单个桶（核心步骤）,对每个桶：

- 如果是链表/树，按 (hash & oldCapacity) 拆成两部分：
- 低位链（lo） → 留在原索引
- 高位链（hi） → 移到 index + oldCapacity
- 将数据分别放入 nextTable

7. 在旧桶位置放置 ForwardingNode

- 表示该桶已迁移完成
- 后续访问会跳转到 nextTable

8. 读写线程协助迁移（helpTransfer）

- get 遇到 ForwardingNode → 去新表查
- put 遇到迁移中 → 帮忙迁移部分桶

9. 持续直到所有桶迁移完成，所有桶都被标记为 ForwardingNode，说明迁移结束。

10.	切换新表为主表，table = nextTable，nextTable = null
