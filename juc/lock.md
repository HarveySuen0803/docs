...
Biased Lock 不存在释放锁这个说法，T1 运行结束后，也不会去修改 Monitor 的 Owner Field 中存储 Cur Thread 的 Id，所以下次 T2 来访问时，不清楚 T1 到底有没有执行完，所以会尝试去撤销 Biased Lock，判断 T1 是否退出了同步块，如果 T1 不在同步块中，则 T2 会直接修改 Monitor 的归属线程 ID 为 T2，如果 T1 还在同步块中，则 T2 会发送 发动 Stop The World !!! 升级为 Light Lock !!! 此时 Thread A, 原先持有的 Biased Lock 会替换为 Light Lock, 继续执行完后, 释放 Light Lock, 两个 Thread 公平竞争 Light Lock
...
Thread t1 = new Thread(() -> {
    ...
    LockSupport.park(); // 提前收到了 permit，不会永久等待，安全
});
t1.start();

Thread t2 = new Thread(() -> {
    LockSupport.unpark(t1); // 提前发放 permit
});
t2.start();
```
...
假设，T1 获取到锁，正在执行修改；T2 也进来了，执行了第一次 Check，生成了一个 Page View，Page View 里存储的是 "old data"；T1 执行完修改，将 "old data" 修改为了 "new data"，释放锁，提交事务；T2 获取到锁，执行第二次 Check 时就会存在问题，由于采用的是 RR 事务隔离级别，当生成一个 Page View 后，就不会再去生成新的 Page View，所以 T2 在第二次 Check 时，查询到的是 "old data"，而非刚刚 T1 修改的 "new data"。
